...
class Server {
    ...
    var url = "https://apiamanda.saludencamino.com/api"
    fun login(user: String, password: String, context: Context): Boolean {
        ...
        var a = runBlocking {
            val (request, response, result) = Fuel.post("$url/usuario/login")
                .jsonBody(body.toString()).awaitStringResponseResult()
            result.fold(
                { data ->
    ...
    fun saveData(context: Context, data: JSONObject): Boolean {
        println(data);
        var a = runBlocking {
            val (request, response, result) = Fuel.post("$url/medicionUsuario/createMedicionesWithIdUsuario")
                .jsonBody(data.toString()).awaitStringResponseResult()
            result.fold(
                { data ->
                    println(data);
